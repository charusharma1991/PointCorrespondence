function [Graph1,Graph2] = RipsEllipse(data1,data2,nv1,nv2,k)
...
for i=1:size(newdata1,1)
    newd1(i,:) = plane2ellipsoid(newdata1(i,:));
end
for i=1:size(newdata2,1)
    newd2(i,:) = plane2ellipsoid(newdata2(i,:));
end
newdata1=newd1;
newdata2=newd2;
...
for i =1:size(newdata1,1)
    for j = 1:size(newdata1,1)
        dis11(i,j) = EllipsoidDist(newdata1(i,1),newdata1(i,2),newdata1(j,1),newdata1(j,2));
    end
end
for i =1:size(newdata2,1)
    for j = 1:size(newdata2,1)
        dis22(i,j) = EllipsoidDist(newdata2(i,1),newdata2(i,2),newdata2(j,1),newdata2(j,2));
    end
end
[~,id1] = sort(dis11,2);
[~,id2] = sort(dis22,2);
