function err= GM_RipsCone_SP(data,N1,N2)
%Input: data cell of size N contains each matrix of (x,y) coordinates, N1 is number of spherical images in a dataset and N2 is number of planar images in dataset to match
...
for m=1:N1
    for n=N1+1:N1+N2
        ...
        nv1 = length(data1);
        nv2 = length(data2);
        [G1,G2] = RipsCone(data1,data2,nv1,nv2,k);
        [~,cl1,~] = k_clique(4,G1);
        [~,cl2,~] = k_clique(4,G2);
        ...
        [P{m}{n},A{m}{n},ptR{m},ptC{n},tim]=matchHasseCone(H1,H2,cliques1,cliques2,coord1,coord2,nodes,nv1,nv2);
        clear cliques1 cliques2 coord1 coord2 data1 data2 nv2 G1 G2 cl1 cl2 nodes H1 H2 count count1
    end
end
err = cRerror_Occ_SP(P,N1,N2,nv1,ptR,ptC);
end
